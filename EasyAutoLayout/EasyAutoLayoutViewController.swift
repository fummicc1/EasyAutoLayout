        ...
        for (index, subview) in subViewsOrderedByTop.enumerated() {
            ...
            if subViewsOrderedByTop.last == subview { // 一番下にあるビューに対しては親と.bottom, １つ上のビューとは.topを制約する。
                ...
                setLayout(
                    ...
                    distance: (self.view.frame.maxY - subViewsSizeOrderedByTop[index].maxY),
                    isVerticle: true
                )
                ...
                setLayout(
                    ...
                    distance: (subViewsSizeOrderedByTop[index].origin.y - self.view.frame.origin.y),
                    isVerticle: true
                )
            ...
            setLayout(
                ...
                from: subViewsOrderedByTop[index-1],
                attributes: (from: bottom, to: top),
                distance: (subview.frame.origin.y - subViewsOrderedByTop[index-1].frame.maxY),
                isVerticle: true
            )
            ...
            setLayout(
                ...
                from: subViewsOrderedByTop[index+1],
                attributes: (from: top, to: bottom),
                distance: (subview.frame.maxY - subViewsOrderedByTop[index+1].frame.origin.y),
                isVerticle: true
            )
            
//            subview.setContentCompressionResistancePriority(.init(699), for: .vertical)
        ...
        for (index, subview) in subViewsOrderedByLeft.enumerated() {
            ...
            if subViewsOrderedByLeft.last == subview { // 一番右にあるビューに対しては親のrightと自分のright, １つ左のビューのrightと自分のleftを制約する。
                ...
                setLayout(
                    ...
                    distance: (subview.frame.maxX - self.view.frame.maxX),
                    isVerticle: false
                )
                ...
                setLayout(
                    ...
                    distance: (subview.frame.origin.x - self.view.frame.origin.x),
                    isVerticle: false
                )
